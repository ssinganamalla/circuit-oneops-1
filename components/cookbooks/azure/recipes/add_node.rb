...
client = AzureCompute::VirtualMachineHelper.new(node)

# check whether the VM with given name exists already
node.set['VM_exists'] = client.check_vm_exists?

# begin
#   # client.virtual_machines.get(resource_group_name, server_name)
#   node.set['VM_exists'] = client.check_vm_exists?(resource_group_name, server_name)
#   OOLog.debug("VM Exists?: #{node['VM_exists']}")
