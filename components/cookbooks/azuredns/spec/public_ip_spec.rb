...
describe 'Azuredns::public_ip' do
  ...
  describe 'PublicIp::update_dns' do
    it 'returns nil if node.app_name is "os"' do
      node['app_name'] = 'os'
      allow(dns_public_ip.aznetwork_public_ip).to receive(:get).and_return(Fog::Network::AzureRM::PublicIp.new)
      allow(dns_public_ip.aznetwork_public_ip).to receive(:create_update).and_return(Fog::Network::AzureRM::PublicIp.new)
  ...
  describe 'PublicIp::update_dns_for_os' do
    ...
    it 'returns not nil if node.full_hostname is not nil' do
      ...
      allow(dns_public_ip.aznetwork_public_ip).to receive(:get).and_return(Fog::Network::AzureRM::PublicIp.new)
      allow(dns_public_ip.aznetwork_public_ip).to receive_message_chain(:create_update ).and_return(Fog::Network::AzureRM::PublicIp.new)
  ...
  describe 'PublicIp::update_dns_for_fqdn' do
    ...
    it 'returns lb-list if availability is "redundant"' do
      ...
      allow(dns_public_ip.aznetwork_public_ip).to receive(:check_existence_publicip) { true }
      allow(dns_public_ip.aznetwork_public_ip).to receive(:get).and_return(Fog::Network::AzureRM::PublicIp.new)
      allow(dns_public_ip.aznetwork_public_ip).to receive(:create_update).and_return(Fog::Network::AzureRM::PublicIp.new)
